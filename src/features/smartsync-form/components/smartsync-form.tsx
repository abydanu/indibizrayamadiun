...
import type { Paket } from '@/features/smartsync-dashboard/types/paket';
import type { Sales } from '@/features/smartsync-dashboard/types/sales';

// Import komponen-komponen baru
import FormStepper from './steps/FormStepper';
import FormNavigation from './steps/FormNavigation';
import Step1InformasiUsaha from './steps/Step1InformasiUsaha';
import Step2DataPIC from './steps/Step2DataPIC';
import Step3PaketSales from './steps/Step3PaketSales';
...
const SmartForm = () => {
  ...
  const fetchPakets = useCallback(async () => {
    ...
    try {
      ...
      const paketData = ((res.data as any).data) || [];
  ...
  const fetchSales = useCallback(async () => {
    ...
    try {
      ...
      const onlyActive = (salesData as Sales[]).filter(s => s.status === 'ACTIVE');
      setSales(onlyActive);
    } catch (error) {
  ...
  const handleSalesSelection = (salesId: string) => {
    ...
    if (selectedSalesData) {
      ...
      const hasCode = !!(selectedSalesData.kode_sales && selectedSalesData.kode_sales.trim() !== '');
  ...
  const nextStep = async (e?: React.MouseEvent) => {
    ...
    if (currentStep === 1) {
      const step1Fields = [
        'nama_usaha', 'datel_pemesanan', 'hp1', 'hp2', 'alamat_pemasangan', 'koordinat_alamat',
        ...(skipFilesGlobal ? [] : ['foto_lokasi', 'bukti_usaha', 'bukti_nib_npwp'])
      ];
      isValid = await validateStepFields(step1Fields);
    } else if (currentStep === 2) {
      const step2Fields = [
        'nama_pic', 'ttl_pic', 'nomor_ktp', 'email',
  ...
  const validateStepFields = async (fieldNames: string[]) => {
    // Andalkan rules dari react-hook-form/useForm (resolver/schema) agar error muncul di field
  ...
  const handleSubmit = async (data: any) => {
    ...
    try {
      ...
      // Validasi dipindah ke react-hook-form rules; di submit kita asumsi field sudah valid
      ...
      if (skipFiles) {
        ...
        // Tanpa kompresi/konversi: kirim file apa adanya (image/non-image)
        // Tidak ada perubahan pada submitData untuk file
      }
      ...
      if (error.response) {
        ...
        const errorMessage = error.message || 'Gagal submit registrasi. Silakan coba lagi.';
