            ...
            {loadingPakets ? (
              ...
              <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    {value && selectedPaket
                      ? `[${selectedPaket.bandwith}] ${selectedPaket.nama} + ${selectedPaket.applied_promos?.map(p => p.nama).join(', ') || 'No Promo'} + (Rp ${Number(selectedPaket.final_price || selectedPaket.price)?.toLocaleString('id-ID')})`
                      : 'Pilih Paket'}
                          ...
                          >
                            ...
                            <div className="flex flex-col">
                              <span className="font-medium">
                                [{paket.bandwith}] {paket.nama} + {paket.applied_promos?.map(p => p.nama).join(', ') || 'No Promo'}
                              </span>
                              <span className="text-sm text-gray-500">
                                (Rp {Number(paket.final_price || paket.price)?.toLocaleString('id-ID')}/bulan)
                              </span>
                            </div>
                          </CommandItem>
                        ))}
