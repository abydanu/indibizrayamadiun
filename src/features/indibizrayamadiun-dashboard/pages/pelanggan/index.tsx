...
import type { Pelanggan, PelangganValidationPayload } from "../../types/pelanggan";
...
export const createColumns = (
  ...
  isSubmitting: boolean
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="font-medium">{row.getValue("datel_nama")}</div>,
    },
    {
      accessorKey: "nama",
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="font-medium">{row.getValue("nama_pic")}</div>,
    },
    {
      accessorKey: "no_hp_1",
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="line-clamp-1 max-w-[260px]">{row.getValue("alamat")}</div>,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="font-medium">{(row.getValue("paket_nama") as string) || "-"}</div>,
    },
    {
      accessorKey: "sales_nama",
      header: () => <span className="font-extrabold">Sales</span>,
      cell: ({ row }) => <div className="font-medium">{(row.getValue("sales_nama") as string) || "-"}</div>,
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return url ? (
          <a href={url} className="text-blue-600 hover:underline" target="_blank" rel="noreferrer">Lihat</a>
        ) : (
          <span>-</span>
        );
      },
    ...
    },
    {
      ...
      header: () => <span className="font-extrabold">Bukti NIB & NPWP Usaha</span>,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge
            variant="outline"
            className={`capitalize font-medium ${isOk
...
export default function ManageRegistrasi() {
  ...
  const formatPaketDisplay = React.useCallback((paket?: PaketLite) => {
    ...
    const promos = paket.applied_promos && paket.applied_promos.length > 0
    ...
    const priceText = priceNumber
      ...
      : "0";
    return `[${band}] ${truncatedName}${promos} (Rp ${priceText})`;
  }, []);
  ...
  const fetchDatels = React.useCallback(async () => {
    try {
      const res = await api.get(`${process.env.NEXT_PUBLIC_API_URL}/datel/list`);
  ...
  const fetchPakets = React.useCallback(async () => {
    try {
      ...
      const list: PaketLite[] = ((res.data as any).data || []).map((p: any) => ({
        id: p.id,
        nama: p.nama,
        bandwith: p.bandwith,
        final_price: p.final_price,
        applied_promos: Array.isArray(p.paket_promos)
          ? p.paket_promos.map((pp: any) => ({ id: pp?.promo?.id, nama: pp?.promo?.nama }))
          : [],
      }));
      setPakets(list);
    } catch (error) {
  ...
  const editFormFields: FormField[] = editing
    ? [
      ...
      },
      {
        ...
        options: pakets.map((p) => ({ value: p.id, label: formatPaketDisplay(p) || p.nama })),
        required: true,
      },
      {
        id: 'edit-sales_id',
      ...
      { id: 'edit-nama', label: 'Nama Usaha', type: 'text', value: editing.nama, onChange: (v) => setEditing((p) => p ? { ...p, nama: v } : p), required: true },
      { id: 'edit-nama_pic', label: 'Nama PIC', type: 'text', value: editing.nama_pic, onChange: (v) => setEditing((p) => p ? { ...p, nama_pic: v } : p), required: true },
      { id: 'edit-email', label: 'Email', type: 'text', value: editing.email, onChange: (v) => setEditing((p) => p ? { ...p, email: v } : p) },
      { id: 'edit-no_hp_1', label: 'No. HP 1', type: 'text', value: editing.no_hp_1, onChange: (v) => setEditing((p) => p ? { ...p, no_hp_1: v } : p), required: true },
      { id: 'edit-no_hp_2', label: 'No. HP 2', type: 'text', value: editing.no_hp_2 || '', onChange: (v) => setEditing((p) => p ? { ...p, no_hp_2: v } : p) },
      { id: 'edit-ttl_pic', label: 'Tempat Tgl Lahir PIC', type: 'text', value: editing.ttl_pic, onChange: (v) => setEditing((p) => p ? { ...p, ttl_pic: v } : p) },
      { id: 'edit-no_ktp', label: 'No KTP/NIK PIC', type: 'text', value: editing.no_ktp, onChange: (v) => setEditing((p) => p ? { ...p, no_ktp: v } : p) },
      { id: 'edit-alamat', label: 'Alamat', type: 'textarea', value: editing.alamat, onChange: (v) => setEditing((p) => p ? { ...p, alamat: v } : p), required: true },
      { id: 'edit-kordinat', label: 'Koordinat', type: 'text', value: editing.kordinat, onChange: (v) => setEditing((p) => p ? { ...p, kordinat: v } : p) },
      {
        id: 'edit-foto_ktp',
        label: 'Foto KTP',
        type: 'custom',
  ...
  React.useEffect(() => {
    // once lookups loaded (non-empty), fetch registrasi to avoid initial dashes
      ...
      <Main>
        <PageTitle
          ...
          <div className="flex gap-2 self-end">
            <Button onClick={exportToExcel} className="px-2 py-1 text-xs sm:px-4 sm:py-2 sm:text-sm w-auto">
              Export Excel
            </Button>
          </div>
        </PageTitle>
